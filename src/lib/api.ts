...
const departmentEmails: Record<string, string> = {
  ...
  hostel:
    'simarjot2221108@gndec.ac.in',

  academic:
    'academic@gndec.ac.in',

  transport:
    'transport@gndec.ac.in',

  financial:
    'accounts@gndec.ac.in',

  infrastructure:
    'maintenance@gndec.ac.in',
...
export const processUnprocessedEmails =
  async (clerkId: string) => {

  // FETCH UNPROCESSED EMAILS
  const { data: emails, error }
    = await supabase
      .from('emails')
      .select('*')
      .eq('processed', false);
  ...
  for (const email of emails) {
    // SKIP DEPARTMENT REPLIES
    if (
      departmentEmailList.includes(
        email.from?.toLowerCase()
      )
    ) {

      console.log(
        'Department reply skipped:',
        email.from
      );
    ...
    // CREATE GRIEVANCE
    const { data: grievanceData, error }
      = await supabase
        .from('grievances')
        .insert({
          user_id: userData.id,
    ...
    await supabase
      ...
      .insert({
        ...
        action_type: 'created_from_email',
...
export const updateGrievanceCategory =
  async (
    grievanceId: string,
    category: string,
    clerkId: string
...
export const forwardGrievance = async (
  grievanceId: string,
  userId: string,
  department: string,
  note: string
  ...
  const { data: grievance, error: fetchError } =
    await supabase
      .from('grievances')
      .select('*')
      .eq('id', grievanceId)
  ...
  // SELECTED DEPARTMENT EMAIL
  const departmentEmail =
    departmentEmails[
      department as keyof typeof departmentEmails
    ];
  ...
  try {
    await fetch(
      'http://localhost:3001/api/send-department-mail',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
    ...
    console.error(
      'Department email sending failed:',
      mailError
    );
  }
