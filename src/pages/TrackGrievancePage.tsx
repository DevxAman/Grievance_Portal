...
const allFilteredGrievances = (
  allGrievancesFilter === 'all'
    ? grievances
    : grievances.filter(
        g => g.status === allGrievancesFilter
      )
).sort(
  (a, b) =>
    new Date(b.created_at).getTime() -
...
const searchedGrievances =
  user?.role === 'clerk' ||
  user?.role === 'dsw'
    ? allFilteredGrievances.filter(
        grievance =>
          grievance.id
            .toLowerCase()
            .includes(
...
<input
  type="text"
  placeholder="Search by Grievance ID (e.g. A96000FB)"
  value={searchId}
  onChange={(e) =>
